...
public class DeptDAO {
  ...
  public boolean updateDept(Dept deptData) {
    try {
      String sqlString = "UPDATE dept " +
	                  	   	 "SET dname = '" + deptData.getDeptname() +"' "+ 
                    			 "WHERE deptno = " + deptData.getDeptid();
  ...
  public Dept findDept(int deptid) {
    try {
      ...
      Statement stmt = conn.createStatement();
      String sqlString = "SELECT department_name " +
	       			  "FROM departments WHERE department_id = " + deptid;
