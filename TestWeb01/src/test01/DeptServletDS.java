...
public class DeptServletDS extends HttpServlet {
  ...
  private void processQuery(HttpServletRequest request, 
    ...
    Dept dept = deptDAO.findDept(Integer.parseInt(deptid));
    if (dept == null) showError(response, " can not find this department no" + deptid);
  ...
  private void showForm(HttpServletResponse response, Dept dept)
    ...
    out.println("Department No:<INPUT TYPE='TEXT' NAME='deptid' VALUE='" +
                dept.getDeptid() + "'><BR>");
    out.println("Department Name:  <INPUT TYPE='TEXT' NAME='deptname' VALUE='" +
                dept.getDeptname() + "'><BR>");
    out.println("  <CENTER>");
